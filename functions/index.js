...
const bucketName = 'imaganes-barbas';
...
exports.dialogflowFirebaseFulfillment = functions.https.onRequest((request, response) => {
  ...
  if (request.body.result) {
    ...
    return response.status(400).end('Salte  de aqui Webhook Request (expecting v1 or v2 webhook request)');
  }
});
/*
...
function processV1Request (request, response) {
  ...
  let number = request.body.result.parameters.number;
  let nombre = request.body.result.parameters.any;
    ...
    'input.unknown': () => {
      ...
      if (requestSource === googleAssistantRequest) {
        sendGoogleResponse(`
          ...
          </speak>`); // Send simple response to user
      } else {
...
const googleRichResponse = app.buildRichResponse()
 ...
 like line  \nbreaks`) // Note the two spaces before '\n' required for a
    ...
    .setImage('https://storage.googleapis.com/imaganes-barbas/beard-'+randnumber()+'.jpg',
...
function randnumber(){
  return Math.floor(Math.random() * 12) + 1758;
}
